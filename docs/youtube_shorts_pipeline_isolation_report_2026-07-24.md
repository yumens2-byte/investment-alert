...
stale patch가 파일 내용과 결합하지 못하도록 충돌 대상 test 경로를 재사용하지 않는다. 다만 외부 CI/로컬 명령이 아직 `run_shorts.py`를 직접 지정할 수 있으므로, CLI 경로는 삭제 대신 안전한 compatibility shim으로 고정한다.

```text
run_shorts.py                 → compatibility shim 유지
run_youtube_shorts.py         → canonical CLI
...
GitHub Actions는 신규 경로만 실행하고 `tests/test_shorts_*.py` 전체를 테스트한다. AST guard는 canonical CLI와 compatibility shim을 모두 검사하고, 폐기된 test 경로가 다시 나타나면 실패한다.
...
- 폐기된 test 경로 2개 부재
- `run_shorts.py` compatibility shim import hygiene 통과
...
- [x] 충돌 test 경로 폐기 및 `run_shorts.py` shim 전환
