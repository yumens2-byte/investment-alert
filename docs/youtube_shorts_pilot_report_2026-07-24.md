...
운영 경로를 `run_youtube_shorts.py`와 `tests/test_shorts_runtime.py`로 교체했고 import를 상단에 한 번만 선언한다. AST 기반 regression test는 신규 경로에서 module-level late import와 동일 `(module, name, alias)` import 중복을 ruff 실행 전에도 차단한다. Workflow의 ruff 단계도 신규 경로만 검사한다.
...
마지막으로 동일 stale patch가 `run_shorts.py`와 `tests/test_shorts_pilot.py`에도 반복 적용되어 두 경로 역시 폐기했다. 실제 CLI는 `run_youtube_shorts.py`, runtime test는 `tests/test_shorts_runtime.py`로 이동했다. GitHub Actions, 문서, AST guard는 모두 신규 경로만 참조하며 구 경로 세 개가 존재하면 별도 테스트가 실패한다.
